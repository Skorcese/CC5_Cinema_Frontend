...
class HomePage extends Component {
  ...
  async addFilms(){
    ...
    if (getMovies.data.length === 0 || getMovies.data.length === null || getMovies.data.length === undefined)
      movies.map( (v) => end.post('api/movies', v));
  }

  async componentDidMount() {
    const photos = await end.get('api/movies');
    this.setState({photosPro:photos.data.slice(0, 6), photosNew: photos.data.slice(7, 13)});
    ...
    return (<div>
      ...
      <div className="photo-home">
        {this.state.photosPro.map( (value, index)=> {
          return <Photo key={index} imageUrl={value.imageUrl} title={value.title}/>
        })}
      </div>
      <br /><br /><br /><br />
      <hr /><br />
